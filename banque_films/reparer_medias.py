...
def telecharger_image_robuste(url, nom_element):
    ...
    print(f"🌍 Téléchargement pour [{nom_element}]...")
    try:
...
# --- PARTIE 1 : LES FILMS ---
print("\n🎬 Vérification des affiches de films manquantes...")
films_data = {
...
for titre, url in films_data.items():
    try:
        ...
        # On vérifie si l'affiche est vide ou si le fichier physique n'existe pas
        if not film.affiche or not os.path.exists(film.affiche.path):
            ...
            print(f"🟢 {titre} a déjà son affiche.")
    except Film.DoesNotExist:
        print(f"⚠️ Le film '{titre}' n'existe pas dans la base.")

# --- PARTIE 2 : LES ACTEURS ---
print("\n🎭 Vérification des photos d'acteurs manquantes...")
acteurs_data = [
...
for act in acteurs_data:
    ...
    try:
        ...
        if not acteur.photo or not os.path.exists(acteur.photo.path):
            ...
            print(f"🟢 {nom_complet} a déjà sa photo.")
    except Acteur.DoesNotExist:
        print(f"⚠️ L'acteur '{nom_complet}' n'existe pas dans la base.")

print("\n✨ Terminé ! Tout ce qui manquait a été inspecté et réparé.")
